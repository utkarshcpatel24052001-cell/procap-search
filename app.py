...
class PDBWrapper:
    ...
    def get_best_chain(self, requested_chain: str) -> Optional[str]:
        """Smart Chain Resolution Engine to prevent KeyError crashes."""
        ...
        try:
            ...
            if chains: return chains[0] # Fallback to first available chain
            return None
...
def run_comprehensive_diagnostic(query_pdb: Path, db_dir: Path, requested_chain: str, sort_method: str) -> Tuple[pd.DataFrame, Dict]:
    ...
    for pdb_file in sorted(db_dir.glob("*.pdb")):
        ...
        # Match Target Chain to Query Chain (or fallback to its own best chain)
    ...
    if not df.empty:
        ...
        # Methodology Sorting Engine
        if sort_method == "RMSD-Based":
            df = df.sort_values("RMSD_Å", ascending=True, na_position='last').reset_index(drop=True)
        elif sort_method == "Sequence Identity":
            df = df.sort_values("Seq_Identity_%", ascending=False).reset_index(drop=True)
        elif sort_method == "Distance Matrix":
            df = df.sort_values("DM_Correlation", ascending=False).reset_index(drop=True)
        else:
            ...
            if not list(db_dir.glob("*.pdb")):
                ...
                if not query_pdb_path:
                    ...
                    if diagnostics["status"] == "error":
                        ...
                        if search_method == "RMSD-Based":
                            primary_score = f"{top_hit['RMSD_Å']:.2f} Å" if not pd.isna(top_hit['RMSD_Å']) else "N/A"
                        elif search_method == "Sequence Identity":
                            primary_score = f"{top_hit['Seq_Identity_%']:.1f}%"
                        elif search_method == "Distance Matrix":
                            primary_score = f"{top_hit['DM_Correlation']:.3f}"
                        else:
                            primary_score = f"{top_hit['Consensus_Score']:.3f} / 1.0"
