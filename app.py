...
def run_comprehensive_diagnostic(query_pdb: Path, db_dir: Path, chain_id: str) -> Tuple[pd.DataFrame, Dict]:
    ...
    if not df.empty:
        ref_meta = REFERENCE_DF[["PDB_ID", "Organism", "Gene_Name", "Function", "Protein_Family"]].copy()
...
with st.sidebar:
    ...
    st.markdown('<div class="info-box"><b>🗄️ Active Database:</b><br>23 verified bacterial capsule proteins across 5 pathogenic species.</div>', unsafe_allow_html=True)
    if st.button("🔄 Initialize Database", use_container_width=True):
        with st.spinner("📥 Syncing PDB structures from RCSB..."):
            ...
            if f: st.warning(f"⚠️ Failed to load {len(f)} structures.")
    ...
    with col_results:
        if execute_btn:
            ...
            if not query_pdb_path:
                ...
                with st.spinner("🔬 Executing alignment matrices..."):
                ...
                if diagnostics["status"] == "error":
                    ...
                    st.warning("⚠️ No homologous structures found. Ensure the chain ID is correct.")
                else:
                    ...
                    # Dashboard Layout
                    st.markdown(f'<div class="{alert_class}">🧬 CONFIDENCE: {confidence} | Score: {top_hit["Consensus_Score"]:.2f}/1.00</div>', unsafe_allow_html=True)
                    
                    with st.container(border=True):
                        st.markdown("### Top Biological Match")
                        col_top1, col_top2, col_top3 = st.columns(3)
                        col_top1.write(f"**PDB ID:** `{top_hit['Target_PDB']}`\n\n**Organism:** {top_hit.get('Organism', 'N/A')}")
                        col_top2.write(f"**Gene:** {top_hit.get('Gene_Name', 'N/A')}\n\n**Function:** {top_hit.get('Function', 'N/A')}")
                        col_top3.write(f"**Family:** {top_hit.get('Protein_Family', 'N/A')}\n\n**RMSD:** {top_hit['RMSD_Å']:.2f} Å" if not pd.isna(top_hit['RMSD_Å']) else "**RMSD:** N/A")
                    
                    with st.container(border=True):
                        st.markdown("### Advanced Physicochemical Profile (Query)")
                        ...
                        m4.metric("GRAVY", f"{diagnostics['query_gravy']:.3f}")
                    
                    st.markdown("### Consensus Output Matrix")
                    display_cols = ["Target_PDB", "Consensus_Score", "Confidence", "Seq_Identity_%", "RMSD_Å", "Organism"]
                    st.dataframe(results_df[display_cols].head(10), use_container_width=True, hide_index=True)
...
with tab2:
    ...
    if 'query_pdb_content' in st.session_state:
        ...
        with col3d:
            st.markdown("### Interactive 3D Topology")
        ...
        st.info("💡 Execute a diagnostic in Tab 1 to generate 3D models and alignments.")

# ==================================================
# TAB 3 & 4: ANALYTICS & DOCS
# ==================================================
with tab3:
    st.subheader("Reference Database Topology")
    if not REFERENCE_DF.empty:
        c1, c2 = st.columns(2)
        with c1: st.plotly_chart(px.pie(REFERENCE_DF, names="Organism", title="Pathogen Distribution", hole=0.3), use_container_width=True)
        with c2: st.plotly_chart(px.bar(REFERENCE_DF["Protein_Family"].value_counts().reset_index(), x="Protein_Family", y="count", title="Protein Families"), use_container_width=True)
        st.dataframe(REFERENCE_DF, use_container_width=True, hide_index=True)

with tab4:
    st.subheader("Algorithm Documentation")
    st.markdown("""
    **1. RMSD-Based Superposition:** Physics-based 3D geometric measurement.
    **2. Sequence Identity:** Evolutionary primary structure alignment.
    **3. Distance Matrix Topology:** Internal Euclidian correlation, highly resilient to structural loop mutations.
    **Consensus Score:** `0.33 × RMSD + 0.33 × SeqID + 0.34 × DM_Corr`
    """)
