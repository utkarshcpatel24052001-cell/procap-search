...
def run_full_pipeline(query_pdb: Path, db_dir: Path, chain_id: str) -> pd.DataFrame:
    ...
    for pdb_file in sorted(db_dir.glob("*.pdb")):
        ...
        s_score = (sum(x == y for x, y in zip(alns[0].seqA, alns[0].seqB)) / max(len(qseq), len(dseq))) if alns else 0.0
        ...
        if final_score > 0:
            results.append({
                ...
                "RMSD_Score": r_score, "Seq_Identity": s_score, "DM_Correlation": d_score,
                "Target_Seq": dseq
            })
...
with tab1:
    ...
    with c_in:
        ...
        if query_mode == "RCSB Database ID":
            pdb_id = st.text_input("Enter 4-letter PDB ID", value="1W0L").strip().upper()
        else:
    ...
    with c_res:
        if execute:
            ...
            if not query_pdb_path: st.error("Failed to prepare query.")
                ...
                if results_df.empty:
                    st.warning("No structurally viable homologs detected in the database.")
                else:
                    ...
                    with st.container(border=True):
                        ...
                        st.markdown("**🔬 Automated Functional Prediction:**")
                        st.write(f"Based on a **{prediction['confidence'].lower()}-confidence** cluster consensus, the query sequence is predicted to function as a **{prediction['function']}**.")
                        st.write(f"It belongs to the **{prediction['family']}** protein family, utilizing an **{prediction['structure']}** structural architecture.")
                        
                        st.markdown("**🧬 Top Biological Match:**")
                        st.write(f"• **Organism:** {top_hit.get('Organism', 'Unknown')}")
                        st.write(f"• **Gene:** {top_hit.get('Gene_Name', 'Unknown')}")
                        st.write(f"• **Consensus Score:** {score:.2f}/1.00")
                    ...
                    st.markdown("### Consensus Topology Output")
