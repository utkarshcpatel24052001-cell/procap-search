"""
ProCap Search Professional Diagnostic Suite v3.1
...
with st.sidebar:
    ...
    if st.button("Initialize/Update Local Database"):
        with st.spinner("Downloading reference structures from RCSB..."):
            ...
            if f:
                st.warning(f"Failed to download {len(f)} PDBs")
            st.success(f"Database synchronized: {len(d)} PDBs ready.")

tab1, tab2, tab3 = st.tabs(
    ["🔍 Structural Diagnostic", "🧪 3D Viewer & Alignments", "📊 Benchmark Analytics"]
)
...
st.markdown(
    ...
    <b>ProCap Search Professional v3.1</b> | High-Throughput Bioinformatics Suite<br>
    Author: Utkarsh Patel | 
