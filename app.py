...
class PDBWrapper:
    ...
    def get_coordinates(self, chain_id: str) -> Optional[np.ndarray]:
    ...
    def get_sequence(self, chain_id: str) -> Optional[str]:
        if not self.structure or not chain_id: return None
        try:
            return "".join([THREE_TO_ONE.get(res.get_resname(), "X") for res in self.structure[0][chain_id]])
        except KeyError: return None
        
    def get_experimental_metadata(self, chain_id: str):
...
def get_advanced_physchem_properties(seq: str) -> Dict[str, float]:
    clean_seq = seq.replace("X", "").replace("U", "") if seq else ""
...
def run_comprehensive_diagnostic(query_pdb: Path, db_dir: Path, requested_chain: str, sort_method: str) -> Tuple[pd.DataFrame, Dict]:
    ...
    physchem_query = get_advanced_physchem_properties(qseq) if qseq else {}
    
    chain_fallback_warning = None
    if q_chain != requested_chain:
        chain_fallback_warning = f"Query chain '{requested_chain}' missing. Auto-resolved to Chain '{q_chain}'."
    ...
    if not df.empty:
        ...
        if sort_method == "RMSD-Based": df = df.sort_values("RMSD_Å", ascending=True, na_position='last').reset_index(drop=True)
        elif sort_method == "Sequence Identity": df = df.sort_values("Seq_Identity_%", ascending=False).reset_index(drop=True)
        elif sort_method == "Distance Matrix": df = df.sort_values("DM_Correlation", ascending=False).reset_index(drop=True)
        else: df = df.sort_values("Consensus_Score", ascending=False).reset_index(drop=True)
...
def generate_alignment_visualization(seqA: str, seqB: str, blocksize: int = 60) -> str:
    if not seqA or not seqB or not isinstance(seqA, str) or not isinstance(seqB, str):
        return "<div class='alert-low'>⚠️ Sequence alignment failed: One or both sequences are missing for the selected chain.</div>"
    ...
    try:
        ...
        return f"<div class='alert-low'>⚠️ Alignment error: {str(e)}</div>"
...
with st.sidebar:
    ...
    chain_id = st.text_input("Target Chain ID", value="A", max_chars=1, help="Pipeline will auto-fallback if chain is missing.")
    
    search_method = st.radio(
        "Scoring Methodology", 
        ["Consensus (Weighted)", "RMSD-Based", "Sequence Identity", "Distance Matrix"],
        help="Select the primary metric for ranking homologs."
    )
            ...
            if not list(db_dir.glob("*.pdb")):
                ...
                if not query_pdb_path:
                    ...
                    with st.spinner("🔬 Executing structural alignments against CSV database..."):
                        results_df, diagnostics = run_comprehensive_diagnostic(query_pdb_path, db_dir, chain_id, search_method)
                    ...
                    if diagnostics["status"] == "error":
                        ...
                        st.warning("⚠️ No homologous structures found matching criteria.")
                    else:
                        ...
                        if search_method == "RMSD-Based": primary_score = f"{top_hit['RMSD_Å']:.2f} Å" if not pd.isna(top_hit['RMSD_Å']) else "N/A"
                        elif search_method == "Sequence Identity": primary_score = f"{top_hit['Seq_Identity_%']:.1f}%"
                        elif search_method == "Distance Matrix": primary_score = f"{top_hit['DM_Correlation']:.3f}"
                        else: primary_score = f"{top_hit['Consensus_Score']:.3f} / 1.0"
                        ...
                        st.markdown(f'<div class="{alert_class}">🧬 CONFIDENCE: {confidence} | {search_method.split(" ")[0]} Score: {primary_score}</div>', unsafe_allow_html=True)
                        ...
                        with st.container(border=True):
                            ...
                            st.markdown("#### Top Reference Hit Analytics")
                            st.write(f"**PDB ID:** `{top_hit['Target_PDB']}` (Chain {top_hit['Target_Chain']}) | **Organism:** {top_hit.get('Organism', 'Unknown')} | **Gene:** {top_hit.get('Gene_Name', 'Unknown')}")
                            
                            t1, t2, t3, t4 = st.columns(4)
                            t1.metric("1. RMSD (3D Physics)", f"{top_hit['RMSD_Å']:.2f} Å" if not pd.isna(top_hit['RMSD_Å']) else "N/A")
                            t2.metric("2. Seq Identity (1D)", f"{top_hit['Seq_Identity_%']:.1f}%")
                            t3.metric("3. Distance Matrix (Topol)", f"{top_hit['DM_Correlation']:.3f}")
                            t4.metric("🏆 Consensus Score", f"{top_hit['Consensus_Score']:.3f}")
                        ...
                        st.markdown(f"### Search Results Topology Matrix (Sorted by {search_method})")
                        display_cols = ["Target_PDB", "Organism", "Protein_Family", "Consensus_Score", "RMSD_Å", "Seq_Identity_%", "DM_Correlation", "Confidence"]
...
with tab2:
    ...
    if 'query_pdb_content' in st.session_state:
        ...
        with col3d:
            st.markdown(f"### Interactive 3D Topology (Chain {st.session_state.get('resolved_chain', 'A')})")
        ...
        with colseq:
            if 'results_df' in st.session_state and not st.session_state['results_df'].empty:
                ...
                qseq = qp.get_sequence(st.session_state.get('resolved_chain', 'A'))
...
with tab4:
    ...
    st.markdown("""
    **1. Smart Chain Resolution Engine:** Automatically falls back to viable molecular chains if user queries missing assemblies.
    **2. RMSD-Based Superposition:** Physics-based 3D geometric measurement across alpha-carbons.
    **3. Sequence Identity:** Evolutionary primary structure alignment via global pairing.
    **4. Distance Matrix Topology:** Internal Euclidian correlation, heavily resilient to structural loop mutations and insertions.
